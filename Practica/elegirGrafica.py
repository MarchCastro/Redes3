...
from graficarRRD import graficar, graficar_HW
...
class VentanaGraficas(object):
	...
	def create_window(self):
		...
		tk.Button(self.window, text='Linea de base', command= lambda: self.inicia_ventana_grafica_LB(1)).grid(row=1, column=1)
		...
		tk.Button(self.window, text='Linea de base', command= lambda: self.inicia_ventana_grafica_LB(2)).grid(row=2, column=1)
		...
		tk.Button(self.window, text='Linea de base', command= lambda: self.inicia_ventana_grafica_LB(3)).grid(row=3, column=1)
		...
		tk.Button(self.window, text='Linea de base', command= lambda: self.inicia_ventana_grafica_LB(4)).grid(row=4, column=1)
		...
		tk.Button(self.window, text='Linea de base', command= lambda: self.inicia_ventana_grafica_LB(5)).grid(row=5, column=1)
		tk.Button(self.window, text='Holt Winters', command= lambda: self.inicia_ventana_grafica_holtW(5)).grid(row=5, column=2)
	...
	def inicia_ventana_grafica_LB(self,id_grafica):
		a = Grafica_LB(self.root,self.rrd_name_LB,self.host+"-"+str(id_grafica)+"_LB.png",id_grafica)		
...
class Grafica_HW(object):
	def __init__(self, root, rrd_name, imagen, id_grafica):
		...
		#Debe ser un hilo por cada grafica iniciada
		t2 = threading.Thread(target=graficar_HW, args=("Graficando...",self.rrd_name,self.imagen,self.id_grafica,))
		t2.daemon = True
		t2.start()
...
class Grafica_HW(object):
	def __init__(self, root, rrd_name, imagen, id_grafica):
		...
		#Debe ser un hilo por cada grafica iniciada
		t2 = threading.Thread(target=graficar_LB, args=("Graficando...",self.rrd_name,self.imagen,self.id_grafica,))
		t2.daemon = True
		t2.start()
		...
		if id_grafica == 1:
			self.window1.title("Linea base para trafico de la interfaz")			
		elif id_grafica == 2:
			self.window1.title("Linea base para conexiones TCP establecidas")
		elif id_grafica == 3:
			self.window1.title("Linea base para segmentos TCP")
		elif id_grafica == 4:
			self.window1.title("Linea base para estadisticas ICMP")
		elif id_grafica == 5:
			self.window1.title("Linea base para respuestas SNMP")
		else:
			self.window1.title("Graficos")	
