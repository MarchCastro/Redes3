...
top.geometry('2200x850')

photoFrame = Frame(top, width=2200, height=800)
photoFrame.grid()
photoFrame.rowconfigure(0, weight=1)
photoFrame.columnconfigure(0, weight=1)

photoCanvas = Canvas(photoFrame, width=2200, height=800)
photoCanvas.grid(row=0, column=5, sticky="nsew")

canvasFrame = Frame(photoCanvas, width=2200, height=800)
...
def getAgentInfo(ip_community):
        ...
        if status_received == 'Activa':
            ...
            for i in range(1, int(interfaces) + 1):
                ...
                if name_interfaces[1] == '0':
                    ...
                    Label(canvasFrame, text=name_interfaces, width=100, fg='black').grid(row=ro, column=3,
                                                                                         sticky="nsew")
            ...
            Tkinter.Button(canvasFrame, text="Graficas", width=10, command=lambda
                name=[computer['community'], computer['port'], computer['ip'],
                      '1.3.6.1.2.1.2.2.1.2.' + str(i)]: graphics(name)).grid(row=r, column=5, sticky="nsew")
            Tkinter.Button(canvasFrame, text="Estado", width=10,
                           command=lambda name=computer['ip']: MostrarEstado(name)).grid(row=r, column=6, sticky="nsew")
            Tkinter.Button(canvasFrame, text="Eliminar", width=10,
                           command=lambda name=computer['ip']: eliminarAgente(name)).grid(row=r, column=7,
                                                                                          sticky="nsew")
            r = r + int(interfaces)
        else:
            ...
            Label(canvasFrame, text='Informacion no disponible', width=50, fg='red').grid(row=ro, column=3,
                                                                                          sticky="nsew")
            Tkinter.Button(canvasFrame, text="Eliminar", width=10,
                           command=lambda name=computer['ip']: eliminarAgente(name)).grid(row=r, column=7,
                                                                                          sticky="nsew")
    ...
    Label(canvasFrame, text='Numero de agentes: ' + str(agentCount), width=25, fg='black').grid(row=1, column=1,
                                                                                                sticky="nsew")
...
def main():
    add = Tkinter.Button(canvasFrame, text="Agregar agente", width=25, command=addClient).grid(row=0, column=0,
                                                                                               sticky="nsew")
    delete = Tkinter.Button(canvasFrame, text="Eliminar agente", width=25, command=deleteClient).grid(row=1, column=0,
                                                                                                      sticky="nsew")
